...
def Pcr (barra):
    ...
    return ((3.14159**2)*E*I)/((largo**2)*1000)

for barra in barras:
    if esfuerzo_barras[barra] > Pcr(barra)/1.3:

        while esfuerzo_barras[barra] > Pcr(barra)/1.3:
            secciones_barras[barra] = (secciones_barras[barra][0] + 0.5, secciones_barras[barra][1] + 0.5)
            
            #print(esfuerzo_barras[barra])
...
#Vuelvo a verificar que no fallen por tension
a = 0
for secciones in secciones_barras:
    #print('La barra', secciones, 'tiene un area de', area(secciones_barras[secciones]), 'mm^2, una inercia de', inercia(secciones_barras[secciones]), 'mm^4 y una rigidez de', rigidez(secciones_barras[secciones], E), 'KN/mm^2')
    tension = fluencia(secciones)
...
if a == 0:
    print('Ninguna barra falla por tension despues del pandeo')

#verifico que ninguna falle por [pandeo]
...
if a == 0:
    print('Ninguna barra falla por pandeo despues de la tension')
print('')
